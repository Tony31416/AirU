...
#x is independent data, y is dependent, ymin is the minumin y could possibly be, ymax is the max
    ...
    xtoss=x[np.invert(i_keep)]
    ytoss=y[np.invert(i_keep)]
    ...
    plt.plot( dy)
    plt.plot([min(x), max(x), max(x), min(x)], [dymin, dymin, dymax, dymax])
...
for fname in f:  #loop throug all the files
    ...
    if ('CO' in data[fid].columns.values): # it's an AirU Sensor
        ...
        #remove outliers
        outlie = whatOutliers(data[fid]['t'],data[fid]['Temp'],-20,50)
        data[fid]['Temp'] = outlie[0]
        ax1[1][0].plot(data[fid]['Temp'],label=fname)
        ax1[1][0].set_xlabel('Time (min)'), ax1[1][0].set_ylabel('T (°C)')
        #data[fid]['Humidity']=whatOutliers(data[fid]['t'],data[fid]['Humidity'],0,100)
        ax1[1][1].plot(data[fid]['t'],data[fid]['Humidity'],label=fname)
        ax1[1][1].set_xlabel('Time (min)'), ax1[1][1].set_ylabel('H (%)')
